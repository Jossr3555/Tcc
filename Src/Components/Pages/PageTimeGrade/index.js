...
const ACTIVITY_TYPES = ['Todos', 'Eletiva', 'Oficina'];
...
export default function TimeGradeScreen() {
    ...
    useEffect(() => {
        ...
        const fetchUserSchedule = async () => {
            ...
            const inscritosOficinas = userData.oficinasInscrito || []; // Usa o nome correto do campo
                ...
                unsubscribeEletivas = onSnapshot(eletivasCollection, (querySnapshot) => {
                    ...
                    querySnapshot.forEach((doc) => {
                        ...
                        if (
                            ...
                            list.push({ ...data, id: doc.id, tipo: 'Eletiva', supervisor: data.Professor });
                        }
                    });
                    resolve(list);
                });
            ...
            const oficinasPromise = new Promise((resolve) => {
                unsubscribeOficinas = onSnapshot(oficinasCollection, (querySnapshot) => {
                    const list = [];
                    querySnapshot.forEach((doc) => {
                        const data = doc.data();
                        if (
                            data.AnoDisponivel === PersonUser.ano &&
                            inscritosOficinas.includes(data.Nome) // Usa o nome correto do campo
                        ) {
                            list.push({
                                ...data,
                                id: doc.id,
                                tipo: 'Oficina',
                                supervisor: data.Professor || data.supervisor,
                                HorarioInicio: data.HorarioInicio,
                                HorarioFim: data.HorarioFim,
                                DiaSemanal: data.DiaSemanal,
                                Nome: data.Nome,
                            });
                        }
                    });
            ...
            Promise.all([eletivasPromise, oficinasPromise]).then(([eletivas, oficinas]) => {
                const combined = [...eletivas, ...oficinas];
        ...
        return () => {
            ...
            if (unsubscribeOficinas) unsubscribeOficinas();
        };
    }, [auth.currentUser, PersonUser.ano]);
